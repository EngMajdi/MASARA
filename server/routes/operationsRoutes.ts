...
const VALID_CATEGORIES = new Set<FeedCategory>(['all', 'ai', 'trips', 'approvals', 'actions', 'verification', 'safety']);
...
operationsRouter.get('/api/operations/events', (req, res) => {
  ...
  const { tripId, recommendationId, eventType, category, since, limit } = req.query;
