...
function createFreshAuthorizedChild() {
  ...
  const legacyParentId = `test-notif-legacy-parent-${freshCounter}`;
  const legacyStudentId = `test-notif-legacy-student-${freshCounter}`;

  db.insert(legacyUsers)
    .values({ id: legacyParentId, name: `Notif Fresh Parent ${freshCounter}`, email, passwordHash: 'x', role: 'parent' })
    .run();
  db.insert(legacyStudents)
    .values({
      ...
      parentName: `Notif Fresh Parent ${freshCounter}`,
      parentPhone: `+968 9800 ${String(freshCounter).padStart(4, '0')}`,
      busId: bus.id,
      busNumber: 'test',
      pickupLat: 23.6,
      pickupLng: 58.4,
