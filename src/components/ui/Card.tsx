...
export const Card: React.FC<CardProps> = ({ padding = 'md', interactive = false, as = 'div', className = '', children, ...rest }) => {
  const Comp = as as any;
  return (
    <Comp
      className={`bg-surface border border-border-default rounded-2xl ${PADDING_CLASSES[padding]} ${
        interactive ? 'transition-colors hover:border-border-strong cursor-pointer' : ''
      } ${className}`}
      {...rest}
    >
      {children}
