...
# Addendum — Phase 9C: Final Real-World UX/UI Quality Audit

Unlike the prior two passes, this was explicitly **not** a feature-gap or code review — it was a live-browser product-experience audit across all four roles, using the real seeded data, hunting specifically for anything that would make MASARA feel confusing, artificial, or untrustworthy to an actual parent/driver/school/admin. Four genuine P0s and two real P1/P2s were found this way — none of them visible from reading source code in isolation — and fixed. Nothing was found requiring a backend architecture decision.

## P0 — Safety / Trust

- **Wrong child shown in a parent's own home screen.** Logging in as the demo parent (أحمد بن سيف البوسعيدي) showed a *third* child, سالم بن فهد الحوسني, whose own `parentName` field named a completely different person (فهد بن سلطان الحوسني). Root cause: `PATCH /api/students/:id/assign-parent` had always written only `parentId`, never syncing `parentName` — so a student reassigned to a new parent account went on displaying the old guardian's name, and (in this case) a stale assignment from earlier session testing had left one demo student linked to the wrong account entirely. Fixed at the source: `legacyStudentRepository.updateParentId` now keeps `parentName` in sync with whichever account `parentId` actually points to (`server/repositories/legacyStudentRepository.ts`, `server.ts`'s assign-parent route), with a new regression test (`tests/services/employeeManagement.test.ts`). The polluted demo row was corrected back to unassigned through the real endpoint (not a raw DB edit). Live-verified: the parent now correctly sees exactly their two real children.
- **Fabricated ETA on the Parent home screen, contradicting the honest child detail view.** The home list and the pre-arrival "get ready, bus arrives in 4 minutes" banner both read `bus.nextStopEtaMins` — a static seeded number that never changes (already documented as such in `docs/LIVE_TRACKING_ARCHITECTURE_AUDIT.md`) — while the same child's detail sheet, built on the real governed journey, correctly said no live trip was active. A parent could be told to go wait at a stop based on a number with nothing real behind it. Fixed: `ParentPortal.tsx` now reads the same real governed-journey source (`getParentJourneys`) the detail sheet already used, for both the home list's ETA chip and the pre-arrival alert trigger; when no real active journey/ETA exists, neither shows a number — no fallback to the static field. Live-verified with and without an active journey.
- **Fabricated AI results presented as live analysis, including a fake ETA broadcast to parents.** Both `/api/ai/optimize-routes` and `/api/ai/detect-reroute` silently served a hardcoded canned response — identical text and numbers every time — whenever the Gemini call was unavailable, with nothing distinguishing it from a fresh analysis. The reroute fallback's canned parent-facing notification claimed a specific, invented ETA ("الوصول المتوقع خلال 6 دقائق") and was broadcast to every parent (`targetRole: 'all'`) even though no analysis had actually run. Fixed: both endpoints now return `isFallback`, which the Admin UI discloses inline ("تعذّر الوصول إلى خدمة الذكاء الاصطناعي المباشرة — هذا تقدير عام..."); the reroute fallback's own copy no longer states a specific time; and its notification is now scoped to `admin` only when it's a fallback, never reaching parents with an unearned claim. Live-verified both disclosures and the notification's corrected `targetRole`.
- **Fabricated KPI on the Admin's first viewport.** The "طلبات موافقة" (pending approvals) tile — one of four numbers on the very first thing an admin sees — was a literal hardcoded `"—"` string, not a real count, in every environment regardless of actual approval-center state. Fixed: a new `usePendingApprovalsCount` hook reads the same `listRecommendations(status: 'pending')` source the Approval Center itself uses. Live-verified the tile now shows a real, changing number.

## P1/P2 — Accessibility

- **Every `interactive` Card with an `onClick` was keyboard- and screen-reader-inoperable.** `Card.tsx`'s `interactive` prop only ever added hover/cursor CSS — the underlying element was a bare `<div onClick>` with no `role`, no `tabIndex`, no keyboard handler. This pattern is used dozens of times across the whole redesign (bus rows, child cards, admin shortcuts, attention items). Fixed at the source: `Card` now adds `role="button"`, `tabIndex={0}`, a focus ring, and an Enter/Space keyboard handler automatically whenever `interactive` + `onClick` are both present — every call site is fixed with no changes needed at the call sites themselves.
- **Driver's per-student board/absent buttons were 36×36px** (below the ~44px minimum touch target guideline), on the primary safety-critical mobile action a driver repeats for every student while possibly multitasking. Sized up to 44×44px and given real `aria-label`s (previously only a `title` tooltip). The swipe gesture remains the primary interaction; these are the accessible tap fallback.

## Reviewed and left as-is (no change needed)

- Driver's student cards distinguish identical names via seat number (unique per bus) rather than an abstract ID — judged sufficient for the real task (boarding the correct physical seat); the full `#XXXX` disambiguation ID remains available in the collapsed governed journey record for anyone who needs it.
- `/api/ai/predict-traffic-eta`'s fallback path includes a hardcoded `confidenceScore` — but it is never rendered anywhere in the UI, and the feature itself is already explicitly framed as a what-if simulator ("اختر حالة الازدحام لعرض التوقعات"), not a live claim. No user-facing honesty risk found; left as a documented note rather than reworked, per this phase's own instruction not to fix what isn't actually broken.
- Admin's "الأشخاص"/"النقل" section cards read as inert in an accessibility-tree-only pass (the same `Card interactive onClick` pattern the a11y fix above addresses) but are genuinely functional — confirmed live by clicking them.
- No dedicated Seat Management screen or global cross-entity search was built — unchanged from the prior pass's documented deferral; neither came up as a real blocker in any of the four end-to-end role journeys tested.

## Regression

931/931 tests passing (one new regression test added), clean `tsc --noEmit`, clean production build, live-verified in-browser across Parent/Driver/School/Admin including a fresh mobile-viewport pass on Driver (the explicitly highest-priority mobile surface) with zero horizontal overflow and zero real console errors on a fresh page load.
